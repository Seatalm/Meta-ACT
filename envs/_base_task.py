...
class Base_Task(gym.Env):
    ...
    def get_asset_state(self):
        """
        Extract asset/object state from the scene.
        Returns a dict with all objects' positions, orientations, and metadata.
        """
        asset_state = {}
        
        # Get all actor attributes from the task object
        for attr_name in dir(self):
            if attr_name.startswith('_'):
                continue
            attr = getattr(self, attr_name, None)
            # Check if it's a SAPIEN Actor
            try:
                if isinstance(attr, sapien.Actor):
                    pose = attr.get_pose()
                    asset_state[attr_name] = {
                        "position": pose.p.tolist(),
                        "quaternion": pose.q.tolist(),  # [x, y, z, w]
                        "type": "actor"
                    }
            except Exception as e:
                pass
        
        print(f"[DEBUG] Extracted asset state for {len(asset_state)} objects: {list(asset_state.keys())}")
        return asset_state

    def merge_pkl_to_hdf5_video(self, env_config=None):
        if not self.save_data:
            return
        ...
        scene_info = {
            "cluttered_table_info": self.record_cluttered_objects,
            "texture_info": {
                "wall_texture": self.wall_texture,
                "table_texture": self.table_texture,
            },
            "table_z_bias": self.table_z_bias,
            "table_xy_bias": self.table_xy_bias if hasattr(self, 'table_xy_bias') else [0, 0],
        ...
        if hasattr(self, 'info') and isinstance(self.info, dict):
            scene_info.update(self.info)
        
        # Add asset state information
        asset_state = self.get_asset_state()
        if asset_state:
            scene_info["asset_state"] = asset_state
        
        process_folder_to_hdf5_video(cache_path, target_file_path, target_video_path, scene_info)

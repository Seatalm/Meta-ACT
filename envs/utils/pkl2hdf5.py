...
def pkl_files_to_hdf5_and_video(pkl_files, hdf5_path, video_path, env_config=None):
    print(f"[DEBUG] pkl_files_to_hdf5_and_video 被调用，env_config={'None' if env_config is None else 'provided'}")
    ...
    with h5py.File(hdf5_path, "w") as f:
        ...
        if env_config is not None:
            print(f"[DEBUG] 即将调用 save_env_config_to_hdf5")
            save_env_config_to_hdf5(f, env_config)
        else:
            print(f"[DEBUG] env_config 为 None，跳过保存")


def save_env_config_to_hdf5(hdf5_group, config_dict):
    """Save environment configuration as a nested group in HDF5 file."""
    print(f"[DEBUG] save_env_config_to_hdf5 被调用，config_dict 类型: {type(config_dict)}")
    try:
        env_group = hdf5_group.create_group("env_settings")
        print(f"[DEBUG] env_settings group 已创建")
        ...
        def recursively_save_config(group, config_data):
            for key, value in config_data.items():
                try:
                    if isinstance(value, dict):
                        sub_group = group.create_group(key)
                        recursively_save_config(sub_group, value)
                    elif isinstance(value, np.ndarray):
                        try:
                            group.create_dataset(key, data=value)
                        except Exception as e:
                            group.attrs[key] = str(value.shape)
                    elif isinstance(value, (list, tuple)):
                        try:
                            arr = np.array(value)
                            group.create_dataset(key, data=arr)
                        except Exception as e:
                            group.attrs[key] = str(value)[:500] if len(str(value)) > 500 else str(value)
                        ...
                        if is_json_serializable(value):
                            ...
                            group.attrs[key] = str(type(value).__name__)
                except Exception as e:
                    pass
        
        recursively_save_config(env_group, config_dict)
        print(f"[DEBUG] env_settings 配置保存完成")
    except Exception as e:
        print(f"[DEBUG ERROR] Error saving env config to HDF5: {e}")


def process_folder_to_hdf5_video(folder_path, hdf5_path, video_path, env_config=None):
    pkl_files = []
    ...
    pkl_files_to_hdf5_and_video(pkl_files, hdf5_path, video_path, env_config)

...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Subtask ID to process as an independent task (e.g., 1 or 2)",
    )
    ...
    independent_task_name = f"{task_name}_subtask{subtask_id}"
    load_dir = os.path.join("../../data", str(task_name), str(setting), f"data_subtask{subtask_id}")

    begin = 0
    print(f'read data from path:{os.path.join("data", load_dir)}')

    target_dir = f"processed_data/{independent_task_name}-{setting}-{expert_data_num}"
